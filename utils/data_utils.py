...
class IMDBDataset(Dataset):
    ...
    def __init__(self, input_directory, tokenizer, apply_cleaning, max_tokenization_length,
        ...
        self.device = device
        self.pre_tokenize_examples()

    def pre_tokenize_examples(self):
        if not os.path.exists(os.path.join(self.positive_path, 'tokenized')):
            os.mkdir(os.path.join(self.positive_path, 'tokenized'))
            ...
            for i in trange(len(self.positive_files), desc='Tokenizing Positive Reviews',
                ...
                tokenized_example = tokenize(text=example,
                ...
                with open(os.path.join(self.positive_path, 'tokenized', file), mode='wb') as f:
                    pickle.dump(obj=tokenized_example, file=f)
        else:
            logging.warning('Tokenized positive reviews directory already exists!')

        if not os.path.exists(os.path.join(self.negative_path, 'tokenized')):
            os.mkdir(os.path.join(self.negative_path, 'tokenized'))
            ...
            for i in trange(len(self.negative_files), desc='Tokenizing Negative Reviews',
                ...
                with open(os.path.join(self.negative_path, 'tokenized', file), mode='wb') as f:
                    pickle.dump(obj=tokenized_example, file=f)
        else:
    ...
    def __getitem__(self, index):
        if index < self.num_positive_examples:
            ...
            with open(os.path.join(self.positive_path, 'tokenized', file), mode='rb') as f:
                tokenized_example = pickle.load(file=f)
            ...
            with open(os.path.join(self.negative_path, 'tokenized', file), mode='rb') as f:
                tokenized_example = pickle.load(file=f)
        else:
            raise ValueError('Out of range index while accessing dataset')

        return torch.from_numpy(np.array(tokenized_example)).long().to(self.device), label
